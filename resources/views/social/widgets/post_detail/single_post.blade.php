    ...
    <div class="panel-body">
        ...
        <div class="pull-right like-box">
            ...
            <a href="javascript:;" class="all_likes" onclick="showLikes({{ $post->id }})">
                <span>{{ $post->getLikeCount() }} @if($post->getLikeCount() > 1){{ 'likes' }}@else{{ 'like' }}@endif
                </span>
            </a>
        </div>
        <div class="clearfix"></div>
                ...
                <div class="navbar-right">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dd1" style="float: right;">
                            <li><a href="javascript:;" onclick="deletePost({{ $post->id }})"><i class="fa fa-fw fa-trash" aria-hidden="true"></i> Eliminar</a></li>
                        </ul>
                    </div>
                </div>
            @endif
        ...
        <div class="comments-title">
            @include('social.widgets.post_detail.comments_title')
        </div>
        <div class="post-comments">

            @foreach($post->comments()->limit($comment_count)->orderBY('id', 'DESC')->with('user')->get()->reverse() as $comment)

                @include('social.widgets.post_detail.single_comment')
        ...
        <div class="media post-write-comment">
            <form id="form-new-comment">
                ...
                <button type="button" class="btn btn-success btn-md pull-right" onclick="submitComment({{ $post->id }})">
                    Enviar !
                </button>
            </form>
        </div>
