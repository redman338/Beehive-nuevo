...
class PostController extends Controller
{
   ...
   public function fetch(Request $request){
        ...
        if ($wall_type == 1){
            ...
            $posts->where('copropiedad_id', $cop_id);
        ...
        return view('social.widgets.wall_posts', compact('posts', 'user', 'wall_type', 'list_type', 'optional_id', 'limit', 'div_location', 'comment_count'));
    }
    ...
    public function single(Request $request, $id){
        ...
        if ($post->copropiedad_id == $cop_id) {
           
            //$can_see = $post->user->canSeeProfile(Auth::id());
            //if (!$can_see) return redirect('/404');
        
        $update_all = $post->comments();
        $update_all = $post->likes();



        return view('social.post', compact('post', 'user', 'comment_count', 'can_see'));
    ...
    public function comment(Request $request){
        ...
        if ($post && !empty($text)){
            ...
            if ($comment->save()){
                $response['code'] = 200;
                $html = View::make('social.widgets.post_detail.single_comment', compact('post', 'comment'));
                $response['comment'] = $html->render();
                $html = View::make('social.widgets.post_detail.comments_title', compact('post', 'comment'));
        ...
        if ($post_comment){
            ...
            if ($post_comment->comment_user_id == Auth::id() || $post_comment->post->user_id == Auth::id()) {
                if ($post_comment->delete()) {
                    $response['code'] = 200;
                    $html = View::make('social.widgets.post_detail.comments_title', compact('post'));
                    $response['comments_title'] = $html->render();
                }
            }
        }
    ...
    public function likes(Request $request){
        ...
        if ($post){
            $response['code'] = 200;
            $html = View::make('social.widgets.post_detail.likes', compact('post'));
    ...
    public function create(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if ($request->hasFile('image')) {
                ...
                if ($file->move('uploads/posts', $file_name)) {
                    $process = true;
                   
                    $fnsave = 'uploads/posts'.'/'.$file_name;
